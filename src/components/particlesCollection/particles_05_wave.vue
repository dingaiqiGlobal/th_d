    ...
    createPlane() {
      ...
      this.planeMaterial = new THREE.ShaderMaterial({
        uniforms: {
          ...
          ucolor: { value: new THREE.Color(0x9ab0f4) },
          bsize: { value: 3 },
        },
        vertexShader: `
        uniform float uTime;
      ...
      const plane = new THREE.Points(planeGeometry, this.planeMaterial);
      plane.rotation.x = -Math.PI * 0.4;
      this.scene.add(plane);
    },
    animate() {
      const elapsedTime = this.clock.getElapsedTime();//获取自时钟启动后的秒数
